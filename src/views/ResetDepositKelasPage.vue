<template>
  <v-main class="pegawaiBackground">
    <div class="content-style">
        ...
        <template>          
          <v-data-table :headers="headers" :items="transaksiDepositKelass" :search="search" :loading="load" loading-text="Loading..." no-data-text="No Data">
            <template v-slot:[`item.actions`]="{ item }">
                <v-icon class="mr-2" @click="resetDepositKelasHandler(item.id_deposit_kelas)">{{ icons.mdiRefresh }}</v-icon>                
            </template>
          </v-data-table>
        </template>
      </v-card>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-container>
                ...
                <v-text-field v-model="form.jumlah_deposit_kelas" label="Jumlah Deposit Kelas" required type="number" :rules="error.jumlah_deposit_kelas"></v-text-field>                
      ...
      <!-- Dialog Reset Deposit Kelas  -->
      <v-row justify="center">      
        <v-dialog v-model="dialogConfirmResetDepositKelas" max-width="500">
          <v-card>
            <v-card-title class="text-h5">Are you sure to reset this deposit kelas?</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="red" text @click="dialogConfirmResetDepositKelas = false" >No</v-btn>
              <v-btn color="green darken-1" text @click="resetDepositKelas">Yes</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-row>
...
<script>
import { mdiPlus, mdiPencil, mdiDelete, mdiEye, mdiPrinter, mdiRefresh } from "@mdi/js";
...
export default {
  data() {
    return {
      ...
      color: "",
      dialogConfirm: false,
      dialogConfirmResetDepositKelas: false,
  ...
  methods: {
    ...
    getDataTransaksiDepositKelas() {
      this.load = true;
      var url = this.$api + "/transaksiDepositKelasKadaluarsa";
      this.$http
        .get(url, {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
    ...
    resetDepositKelas() {
      var url = this.$api + "/transaksiDepositKelasReset/" + this.resetId;
      this.$http
        .put(url,{}, {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
    ...
    resetDepositKelasHandler(id_deposit_kelas) {
      this.resetId = id_deposit_kelas;
      this.dialogConfirmResetDepositKelas = true;
    },
    getHandler(item) {
      this.editId = item.id_deposit_kelas;
    ...
    generatePDF(item) {         
      ...
      doc.text(`No Struk: ${item.id_deposit_kelas}`, 10, 20);
      doc.text(`Tanggal: ${item.tanggal_deposit_kelas}`, 10, 30);
      doc.text(`Kasir: ${item.id_pegawai} / ${this.selectedPegawai.nama_pegawai}`, 10, 40);      
      doc.text(`Member: ${item.id_member} / ${this.selectedMember.nama_member}`, 10, 50);
      doc.text(`Deposit (bayar ${item.jumlah_deposit_kelas} gratis ${item.bonus_deposit_kelas}): Rp.${item.jumlah_pembayaran_deposit_kelas},- (${item.jumlah_deposit_kelas} x Rp.${this.selectedKelas.harga_kelas})`, 10, 60);
      doc.text(`Jenis Kelas: ${this.selectedKelas.nama_kelas}`, 10, 70);      
      doc.text(`Total Deposit ${this.selectedKelas.nama_kelas}: ${item.total_deposit_kelas}`, 10, 80);
      doc.text(`Berlaku sampai dengan: ${item.tanggal_kadaluarsa_deposit_kelas}`, 10, 90);
