  ...
  <v-main class="pegawaiBackground">
    <div class="content-style">
      ...
      <v-card>
        ...
        <template>          
          <v-data-table :headers="headers" :items="laporanAktivitasKelasBulanans" :search="search" :loading="load" loading-text="Loading..." no-data-text="No Data">
...
export default {
  data() {
    return {
      ...
      laporanAktivitasKelasBulanans: [],
      laporanAktivitasKelasBulanan: new FormData(),      
      ...
      items: [
        { text: "...", disabled: true },
        { text: "Laporan Aktivitas Kelas Bulanan", disabled: false },
      ],
      headers: [
  ...
  mounted() {
    this.getDataLaporanAktivitasKelasBulanan();
  },
  methods: {
    getIdPegawai() {
    ...
    getDataLaporanAktivitasKelasBulanan() {
      this.load = true;
      var url = this.$api + "/laporanAktivitasKelasBulanan";
      this.$http
        .get(url, {
          headers: {
        ...
        .then((response) => {
          this.laporanAktivitasKelasBulanans = response.data.data;          
          this.load = false;
        });
      this.load = true;
    },
    ...
    generatePDF(item) {            
        ...
        doc.text("LAPORAN AKTIVITAS KELAS Bulanan", 10, 30);
        doc.text(`Tanggal Cetak: ${tanggalCetak}`, 10, 40);

        doc.text(`Kelas: ${item.kelas}`, 10, 50);        
        doc.text(`Instruktur: ${item.instruktur}`, 10, 60);
        doc.text(`Jumlah Peserta: ${item.jumlah_peserta}`, 10, 70);
        doc.text(`Jumlah Libur: ${item.jumlah_libur}`, 10, 80);

        doc.save("laporan_aktivitas_kelas_bulanan.pdf");
    },
    ...
    generatePDFAll() {
        ...
        doc.autoTable({
            startY: 50,
            head: [["Kelas", "Instruktur", "Jumlah Peserta", "Jumlah Libur"]],
            body: this.laporanAktivitasKelasBulanans.map(item => [item.kelas, item.instruktur, item.jumlah_peserta, item.jumlah_libur]),
